...
import { Dirent, opendirSync, readFileSync, writeFile } from "fs"
...
function main() {
    const originalDir = opendirSync('benchmarks/NPB-c');
    let currFile: Dirent | null = null;
    const data: string[][] = [['File Name', 'Line', 'Col', 'Loop Header', 'Original Parallelized', 'Cetus Parallelized', 'SrcMP Parallelized']];
    while (currFile = originalDir.readSync()) {
